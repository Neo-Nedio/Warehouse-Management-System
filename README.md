...
- **总代码行数**：**3854行**
- **主要分布**：
  - **Controller层**：887行 (表现层)
  - **Service层**：1400行 (业务逻辑层)
  - **数据模型层**：650行 (Entity/DTO/VO)
  - **工具&配置**：450行 (工具类、配置类)
  - **安全权限**：300行 (认证、授权)
  - **定时任务**：320行 (Quartz、监听器)
  - **其他**：347行 (测试、资源文件)
...
### 7. Quartz 定时任务 - 周报自动发送
- **Spring Quartz 集成**：配置化定时任务管理
- **周报自动发送**：每周一早上9点自动执行
- **智能分发**：自动发送带附件邮件给每个仓库的管理员
---
